...
#
# Used to sort ports; understands basic 1<=>2 as well as CatOS's module/port
# format.
#
sub portSort {
	# pure numerical sort, if possible
	return $a <=> $b if not ($a =~ /\D/ or $b =~ /\D/);

	# probably CatOS module/port format...
	$a =~ /(\d+)\/(\d+)/; my ($modA, $portA) = ($1, $2);
	$b =~ /(\d+)\/(\d+)/; my ($modB, $portB) = ($1, $2);

	# module number takes precedence
	return $modA <=> $modB if $modA ne $modB;
	return $portA <=> $portB;
}
...
sub generateInterSwitch {
	...
	foreach my $swname (keys %switches) {
		...
		foreach my $p (keys %ports) {
			...
			# create the link
			print F "\t$dev -> $swname [ dir=both,color=black,style=$style,label=\"$p - $pno\" ]\n";
		}
	}
...
sub generateSwitches {
	...
	foreach my $swname (keys %switches) {
		...
		foreach my $p (sort portSort keys %ports) {
			my %p = %{ $ports{$p} };
			my $vlans = "";
...
sub generateSwitchGraphs {
	...
	foreach my $swname (keys %switches) {
		...
		foreach my $p (keys %ports) {
			...
			# create the link
			$swname=~ s/-//g; $dev=~ s/-//g;
			print F "\t$dev -> $swname [ dir=both,color=black,style=$style,label=\"$p - $pno\",fontsize=6 ]\n";
		}
		print F "}\n";
		close F;
